...
class GPT2BaseModel(BaseModel):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self._consumed_samples += batch_size * max(self.trainer.num_devices, 1)  # batch size * data parallel size
        labels = inputs["labels"]
        if labels is not None:
            self._consumed_tokens += len(labels.flatten()) * max(self.trainer.num_devices, 1)
        else:
            self._consumed_tokens += len(input_ids.flatten()) * max(self.trainer.num_devices, 1)
